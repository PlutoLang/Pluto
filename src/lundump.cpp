...
typedef struct {
  ...
  bool lua54;  /* we're loading bytecode generated by Lua 5.4 (Pluto 0.8.x-0.9.x) */
} LoadState;
...
static size_t loadVarint (LoadState *S, size_t limit) {
  ...
  do {
    ...
    x = (x << 7) | (b & 0x7f);
  } while (S->lua54 ? ((b & 0x80) == 0) : ((b & 0x80) != 0));
  return x;
}
...
static void checkHeader (LoadState *S) {
  ...
  if (format == LUAC_FORMAT) {  /* Lua bytecode? (or Lua-compatible Pluto bytecode) */
    ...
    if (version == 0)
      S->lua54 = true;  /* This was generated by Pluto 0.8.x or 0.9.x, which are based on Lua 5.4. */
    else if (version != 1)
      error(S, "version mismatch");
  }
  else
